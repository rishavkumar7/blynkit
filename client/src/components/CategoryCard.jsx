...
const CategoryCard = ({ openCategorySpace, openEditSpace, openConfirmBox, category }) => {
    const handleOpenCategoryButtonClick = () => {
        openCategorySpace(category)
    }
    ...
    return (
        <div className="w-full h-full flex flex-col items-center justify-center shadow-[0_0_10px_1px_rgba(0,0,0,0.2)] bg-white rounded-lg">
            <div onClick={ openCategorySpace ? handleOpenCategoryButtonClick : null } className={ `relative group w-full flex items-start justify-center aspect-[0.9] sm:aspect-[1.0] md:aspect-[1.1] lg:aspect-[1.3] overflow-hidden rounded-lg ${ openCategorySpace ? "cursor-pointer" : "cursor-default" }` } >
                <img src={ category.image } alt={ category.name } className="rounded-lg scale-115 sm:scale-105 object-cover object-[center_5px] sm:object-[center_0] -translate-y-4"/>
                {
                    (openEditSpace || openConfirmBox) && (
                ...
                <div className="absolute inset-0 bg-[linear-gradient(to_top,_rgba(0,0,0,1)_8%,_transparent_50%)] p-1 text-sm sm:text-md md:text-lg text-white font-semibold flex items-end justify-center rounded-lg">
                    <p>{ category.name }</p>
                </div>
            </div>
        </div>
